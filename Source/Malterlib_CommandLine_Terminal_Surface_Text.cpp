...
namespace NMib::NCommandLine
{
	...
	// Writes plain text without escapes or line breaks, clipped to _ClipArea (the whole surface if invalid).
	// Combining characters attach to the previous cell. Returns the column after the last cell.
...
#endif
			// A glyph past the clip ends the run. Combining marks are taken above this, so the
			// ones that follow the last glyph still attach to it instead of being cut with the run
					...
					, [&](CStrPtr const &_String) -> bool
					{
						for (auto iParse = _String.f_GetUnicodeIterator(); iParse; ++iParse)
						{
...
#if DCommandLineSupportOverflow
							// Zero width wherever it falls: a mark off the surface must not advance the
							// column either, or the visible rest of the line shifts
							if (fg_CharIsCombining(Char))
							{
								if (pPreviousChixel)
								{
...
#endif
							// A glyph past the destination ends the line; the marks that follow the last
							// glyph were taken above and stay attached to it
							...
							if (SurfaceArea.f_Contains(OutputPosition) && DestinationArea.f_Contains(OutputPosition))
							{
								...
								// Each base codepoint occupies one cell; double-width glyphs are unsupported.
								pPreviousChixel = &Chixel;
						...
						_Change.f_Visit
							(
								[&]<typename tf_CType>(tf_CType const &_Change)
								{
									...
									{
										// Restore the call's text defaults; clearing overrides would expose prior destination-cell styling.
										NextProperties = _Options.m_Properties;
									}
